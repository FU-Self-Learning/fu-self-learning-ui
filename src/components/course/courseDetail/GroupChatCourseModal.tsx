...
const GroupChatCourseModal: React.FC<GroupChatCourseModalProps> = ({
  visible,
  onClose,
  loading,
  courses,
  ...
  return (
    <Modal title='My Enrolled Courses' open={visible} onCancel={onClose} footer={null} width={600}>
      {loading ? (
        <Spin />
      ) : (
        <List
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                    <List.Item.Meta
                      ...
                      description={
                        <>
                          <div>
                            Instructor: <b>{course.instructor?.username}</b>
                          </div>
                          <div>Description: {course.description}</div>
                          <div>
                            Enrollment Date: {new Date(item.enrolledAt).toLocaleDateString('vi-VN')}
                          </div>
                        </>
                      }
                    />
