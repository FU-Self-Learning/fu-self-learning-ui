  ...
  const renderComments = (commentList: CommentResponse[], level = 0) => {
    ...
    return commentList.map((comment) => {
      const indentationClass =
        level > 0 ? `ml-${level * indentationUnit} border-l pl-2 border-gray-200` : '';
        ...
        <div key={comment.id} className={`mb-4 ${indentationClass}`}>
          ...
          <div className='flex gap-4 mt-1'>
            <Button
              ...
              onClick={() => handleReplyClick(comment.id, comment.user?.username || 'Unknown User')}
            >
              Reply
            </Button>
          </div>
    ...
    >
      <div className='flex flex-col h-full'>
        <div className='border-b pb-4 mb-4'>
          ...
          <Typography.Title level={4} className='!mb-2 !text-lg !font-semibold'>
            {post?.title}
          </Typography.Title>
