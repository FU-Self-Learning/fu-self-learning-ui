    ...
    <div className="flex justify-center items-end gap-4 mt-10">
      {topUsers
        ...
        .map((user) => (
          <div
            key={user.rank}
            className={`rounded-xl text-center px-6 py-4 w-52 shadow-md ${
              user.rank === 1 ? "scale-110" : "scale-100"
            } bg-[#1e1e2f]`}
          >
            <Image
              preview={false}
            ...
            <div className="text-cyan-300 text-lg font-bold mt-2">
              {user.prize.toLocaleString()} 💎
            </div>
          </div>
        ))}
    </div>
