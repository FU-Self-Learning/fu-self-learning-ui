...
export const CourseTopicsTab = ({ courseId }: CourseTopicsTabProps) => {
  ...
  const [idTopicForLessons, setIdTopicForLessons] = useState<string | null>(null);
  const { mutate: createTopic, isPending: isCreating } = useCreateTopic(courseId);
  const { mutate: updateTopic, isPending: isUpdating } = useUpdateTopic(courseId, topicToEdit?.id);
  const { mutate: deleteTopic, isPending: isDeleting } = useDeleteTopic(courseId);
  ...
  return (
    <div>
      ...
      <LessonManagementModal
        ...
        topicTitle={topics?.find((topic) => topic.id.toString() === idTopicForLessons)?.title || ''}
