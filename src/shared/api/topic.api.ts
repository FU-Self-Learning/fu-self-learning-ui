...
export const updateTopicInstructor = async (
  ...
  const response = await api.put(`${APP_URL}/courses/${courseId}/topics/${topicId}`, request);
  return response.data;
};

export const deleteTopicInstructor = async (courseId: string, topicId: string): Promise<void> => {
