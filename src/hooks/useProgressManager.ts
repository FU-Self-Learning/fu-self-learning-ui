  ...
  const updateProgressOnLessonStart = useCallback(
    (
      ...
      if (currentProgress && currentProgress >= 100) {
        console.log(
          `🚫 useProgressManager: Course ${courseId} already completed (${currentProgress}%), skipping lesson start update`,
        );
        return {
      ...
      console.log(
        `🔢 Calculated progress for lesson ${lessonId}: ${progressData.progressPercentage}%, current DB progress: ${currentProgress}%`,
      );
      ...
      if (currentProgress && progressData.progressPercentage <= currentProgress) {
        console.log(
          `🚫 Progress not increased (${progressData.progressPercentage}% <= ${currentProgress}%), skipping update`,
        );
        return progressData;
      }

      console.log(
        `⬆️ Updating progress from ${currentProgress}% to ${progressData.progressPercentage}% for lesson ${lessonId}`,
      );
      ...
      updateProgress(
        ...
        },
        {
          onSuccess: () => {
            console.log(
              `Progress updated to ${progressData.progressPercentage}% for lesson ${lessonId}`,
            );
          },
          onError: (error) => {
